...
export const generateBatchPDF = async (
    ...
    const filename = `${safePatientId}_capillaroscopy_report_${timestamp}.pdf`;
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            if (file.annotations?.roi) {
                const { roi, scale: fileScale, rotation = 0 } = file.annotations;
                ...
                // Rotate around the center of the ROI
                const cx = roi.x + boxSize / 2;
                const cy = roi.y + boxSize / 2;

                ctx.translate(cx, cy);
                ctx.rotate((-rotation * Math.PI) / 180); // Rotate opposite to image rotation
                ...
                ctx.fillRect(-boxSize / 2, -boxSize / 2 - 25, 120, 25);
                ctx.fillStyle = 'white';
                ctx.font = '14px Arial';
                ctx.fillText('Assessment Box', -boxSize / 2 + 5, -boxSize / 2 - 7);
