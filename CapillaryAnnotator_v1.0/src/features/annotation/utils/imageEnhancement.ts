...
/**
 * Manually adjusts brightness and contrast of an ImageData object
 */
function applyManualSettings(imageData: ImageData, brightnessOffset: number, contrastFactor: number): ImageData {
    ...
    for (let i = 0; i < data.length; i += 4) {
        for (let c = 0; c < 3; c++) {
            ...
            enhanced[i + c] = Math.max(0, Math.min(255, Math.round(val)));
        }
        enhanced[i + 3] = data[i + 3]; // Keep original alpha
    }
    return new ImageData(enhanced, width, height);
}

/**
 * Applies only CLAHE processing without auto-levels or sharpening,
 * but applies a manual adjustment to make it darker with higher contrast.
 */
export async function applyClaheOnly(imageFile: File, clipLimit: number = 2.5, tileSize: number = 8): Promise<string> {
    return new Promise((resolve, reject) => {
        const img = new Image();
        const reader = new FileReader();
        ...
        reader.onload = (e) => {
            img.onload = () => {
                try {
                    ...
                    // Apply only CLAHE
                    imageData = applyCLAHE(imageData, clipLimit, tileSize);
                    
                    // Post-process to make it darker overall and higher contrast
                    // Shift brightness down by 40, multiply contrast by 1.3
                    imageData = applyManualSettings(imageData, -40, 1.3);
